...
flatpak_dir_get_default_locale_languages (FlatpakDir *self)
{
  ...
  g_autoptr(GDBusProxy) accounts_proxy = NULL;
  g_auto(GStrv) default_languages = NULL;

  /* If self is user or system-wide, get the default-languages */
  default_languages = flatpak_dir_get_config_strv (self, "xa.default-languages");

  if (flatpak_dir_is_user (self))
    return flatpak_strv_merge (default_languages, flatpak_get_current_locale_langs ());
  ...
  return flatpak_strv_merge (default_languages, (char **) langs->pdata);
}
